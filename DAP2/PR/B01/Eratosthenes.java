  ...
  public static void main(String[] args) {
    ...
    if(args.length > 0 && args.length < 3){
      try{
        ...
        boolean[] isPrim = sieve(n); //Anlegen eines Arrays des Types boolean mit der Laenge des ausgelesen Parameters
        ...
        if(args.length > 1){ //Ueberpruefen ob es einen weiteren Parameter gibt
          if(args[1].equals("-o")){ //Ueberpruefen ob der zweite Parameter -o ist
            int primeCount = 0;
            for(int j = 2; j < n; j++){ //Array durchlaufen und alle Integers die true sind ausgeben
              if(isPrim[j] == true){
          ...
          int count = 0;
          for(int l = 2; l < n; l++){ //Arrasy durchlaufen und alle Primzahlen zaehlen
            if(isPrim[l] == true){
              count++;
            }
          }
          System.out.println("Die Anzahl der Primzahlen betraegt: " + count); //Ausgabe der Anzahl
        }
      }
